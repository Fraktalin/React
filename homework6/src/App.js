...
function App() {
  ...
  const getDataHandler = () => {
    axios
      .get(`https://5dd3d5ba8b5e080014dc4bfa.mockapi.io/stickers`)
      .then((response) => setNotes(response.data));
  };

  const deleteHandler = (id) => {
    axios
      .delete(`https://5dd3d5ba8b5e080014dc4bfa.mockapi.io/stickers/${id}`)
      .then(() => getDataHandler());
  };

  const editHandler = (text, id) => {
    axios
      .put(`https://5dd3d5ba8b5e080014dc4bfa.mockapi.io/stickers/${id}`, {
        description: text,
      })
      .then(() => getDataHandler());
  };
  ...
  const newNoteSubmit = (event) => {
    ...
    inputHandler();
    axios
      .post(`https://5dd3d5ba8b5e080014dc4bfa.mockapi.io/stickers`, {
  ...
  const toggleTheme = () => {
    setCurrentTheme((prevTheme) =>
      prevTheme === themes.white ? themes.black : themes.white
    );
  };
      ...
      <div className="app" style={{ background: currentTheme.bodyBackground }}>
        <div className="notes_wrap">
          <div className="notes_header">
            <button className="btn new_btn" onClick={inputHandler}>
              New
            </button>
